    ...
    var KuaiyiGenerator = (function () {
        ...
        var generateHTML = function (fm) {
            ...
            for (var a = 0; a < bets.length; a++) {
                ...
                table += '<td>' + no + (xian ? '<span class="red">现</span>' : '') + '</td>';
        ...
        var analyseAllList = function (list) {
            ...
            // 在输入框输入的金额
            var inputedMoney = $('.moneyInput').val().trim();
            if (inputedMoney == 0 || !inputedMoney) {
                inputedMoney = null;
            }
            ...
            inputedMoney = $('.moneyInput').val().trim() || false;
            if (inputedMoney == 0) {
                inputedMoney = false;
            }
            return {
                ...
                totalBetMoney: ((totalBetMoney || (inputedMoney ? result.length * inputedMoney : 0)) - 0).toFixed(1),
                bets: result,
                hasInputMoney: inputedMoney
            };
        };
        ...
        var exceptionRule = function (obj, str, dicNoType) { //(getrule ,  RowsList[i])
            ...
            $.each(checkPos, function (key, value) {
        ...
        var getNumberDigits = function (str) {
            ...
            $.each(returnInfo.result, function (k, v) {
                if (v != null) {
                    returnInfo.result[k] = v.replace(/\s/g, '');//去空格
                }
            });
        ...
        var findErXiongDi = function (str, dicNoType) {
            var returnInfo = {};
            $.each(GRAMMAR_RULE_GAME_RESTRICTIONS[dicNoType], function (key, value) {
        ...
        //多行帶入金額
        var moneyAssign = function() {
            var getMoney = [];
            $.each(AllList, function (key, value) {
                getMoney.push(value.money);
            });
            var finalMoney;
            for (var i = getMoney.length; i > 0; i--) {
                if (getMoney[i] != null) {
                    finalMoney = getMoney[i];
                }
                if (AllList[i - 1].money == null) {
                    AllList[i - 1].money = finalMoney;
                }
            }
        }
        ...
        var replaceMovePosText = function (match, key1, key2) {
            if (key1 == undefined) {
                ...
                var returnText = '';
                $.each(numberPos, function (value, key) {
                    if (key == 0) {
                        returnText += 'x';
                    } else {
                        returnText += '+';
        ...
        $.each(GRAMMAR_RULE_OPEN, function(key, value) {
        ...
        return function (maker, input, output) {
            ...
            var onInput = (function (e) {
                ...
                for (var i = 0; i < RowsList.length; i++) {
                    ...
                    if (!checkNumberOrString) {
                        ...
                        //只有一種玩法則直接帶入定位
                        $.each(SINGLE_PLAY_TYPE, function (key, value) {
                            var regSingle = new RegExp(key);
                            if (regSingle.test(RowsList[i])) {
                                if (codeMakerChoose == 40 || codeMakerChoose == '') {//只有四定或是未選擇定位的才可以直接用專屬玩法判斷
                                    codeMakerChoose = value;
                                } else {//四定出現双双重/值範圍 則報錯
                                    return showError();
                                }
                            }
                        });
                        ...
                        if (codeMakerChoose == '') {
                            ...
                            if (!regPosCheck.test(RowsList[i]) && getPosNum == 0) { //根據千百十个進行定位判斷
                                $.each(getNumberDigitsObj.result, function (key, value) {
                                    if (value != null) {
                                        getPosNum++
                                    }
                                });
                        ...
                        $.each(getNumberDigitsObj.result,
                            function(key, value) {
                        ...
                        $.each(RE_PLACER,
                            function (key, value) {
                                //調整看看
                                //if (key == '双' && RowsList[i].indexOf('双重') != -1) {
                                //    RowsList[i] = RowsList[i].replace(/^双(?!双|重)/g, value);
                                //} else {
                                //    var newReg = new RegExp("([^千百十个五" + key + "])" + key, 'g');
                                //    console.log('RowsList[i]aaa ', RowsList[i]);
                                //    console.log('newReg ', newReg);
                                //    RowsList[i] = RowsList[i].replace(newReg,  "$1" + value);
                                //}
                                regAssign = new RegExp("((两|三)数合|上奖|包含|排除|全转)" + key, 'g');
                        ...
                        //檢查語法規則是否符合玩法
                        $.each(GRAMMAR_RULE_GAME_RESTRICTIONS[getDicNoType],
                            function(key, value) {
                                if (value == false) {
                        ...
                        $.each(GRAMMAR_RULE_GAME_SPECIAL.noneSameTimeUse, function (key, value) {
                            var countCheck = 0;
                        ...
                        $.each(getRule,
                            function (key, value) {
                                if (value[0] != -1) { //先過濾沒有匹配的玩法
                                    if (value[0] == 1) { //需判斷是否該項目有除的選項
                                        if (GRAMMAR_RULE_GAME_SPECIAL.noneExcept.indexOf(key) != -1) { //該玩法在沒有除的列表中, 因此邏輯錯誤
                                            return showError();
                                        }
                                    }
                                    if (value[1] != null) { //玩法後面帶著數字
                                        if (GRAMMAR_RULE_GAME_SPECIAL.noneInt.indexOf(key) != -1) { //該玩法在後續沒有數字, 因此邏輯錯誤
                                            return showError();
                                        }
                                    }
                                    if (GRAMMAR_RULE_GAME_SPECIAL.noneExcept.indexOf(key) != -1) {
                                        if (DICT[key].length == 2) {
                                            var exceptionFliter = key == '两数合' ? 2 : key == '三数合' ? 3 : value[0];
                                            setOption(DICT[key][0], exceptionFliter); // 除|取
                                        ...
                                        if (DICT[key].length == 2) {
                                            setOption(DICT[key][0], value[0].toString()); // 除|取
                                            if (key == '对数' || key == '固定合分' || key == '大' || key == '小' || key == '单' || key == '双') {
                                                setOption(DICT[key][1], value[1]); //对数, 固定合分, 大, 小, 單, 雙比較特別
                                            } else if (key == '配') {
                                                setOption(DICT[key][1], 1); // 配跟定位是使用postionType進行分別, 0: 定位置(預設); 1: 配
                                                //將value 帶入對應位置
                                                var result = {
            ...
            bindInputEvent(input, onInput);

            if (output) {
                outputElement = output;
            }
            // 错误提示
            errTipSpan = $('#input_error');
            return {
                getBets: function () {
                    ...
                    for (var b = 0; b < bets.length; b++) {
                        ...
                        if (bb.bet_money && bb.bet_money != 0) {
                            resultBets.push({
                                bet_money: bb.bet_money,
                                bet_no: bb.bet_no,
                                dict_no_type_id: getTypeId(bb.bet_no, bb.isXian)
                            });
                        }
                    }
                    return {
                        ...
                        bets: JSON.stringify(resultBets),
                        way: 108,
                        period_no: G.instance.header.period_no,
                        bet_log: encodeURIComponent($(input).val())
                    };
                },
